...
| **A2A (Agent2Agent) protocol** | Two-way A2A v1.0 support: `senpi a2a-server` serves sessions over JSON-RPC + SSE with a public agent card, bearer auth, and per-context session reuse; the builtin `a2a` extension turns `a2a.json` agents into `a2a_<name>` tools with a `/a2a` command. | [`docs/a2a.md`](packages/coding-agent/docs/a2a.md) · [`modes/a2a-server/`](packages/coding-agent/src/modes/a2a-server/) |
