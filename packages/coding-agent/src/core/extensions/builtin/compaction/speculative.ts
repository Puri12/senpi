...
import { generateJevCompaction, JevCompactionError } from "./jev/generator.ts";
import type { ResolvedJevCompactionSettings } from "./jev/settings.ts";
...
export interface SpeculativeCompactionSnapshot {
	...
	tools?: Tool[];
	/**
	 * When present, the snapshot is generated by Jev (verbatim prune of tool
	 * calls/results) instead of LLM summarization. Every route builds its
	 * snapshot through the same helper, so this is the single switch.
	 */
	jev?: JevCompactionRoute;
}

/** Jev generator binding carried on a snapshot. */
export interface JevCompactionRoute {
...
export async function runExtensionCompaction(
	...
	if (snapshot.jev) return await runJevCompaction(snapshot, snapshot.jev, signal, onProgress);
...
/**
 * Jev route shared by every snapshot origin. Resolves `undefined` only on
 * abort, mirroring the LLM route; every other failure surfaces as a
 * {@link SummaryGenerationError} (`empty-summary` for a span Jev could not
 * shrink enough, so the deterministic fallback and breaker treat it exactly
 * like a summarizer that returned nothing) or as the transport error itself.
 */
async function runJevCompaction(
	snapshot: SpeculativeCompactionSnapshot,
	jev: JevCompactionRoute,
	signal?: AbortSignal,
	onProgress?: CompactionProgressCallback,
): Promise<CompactionResult | undefined> {
	if (!jev.settings.apiKey) {
		throw new SummaryGenerationError(
			"auth",
			"Jev compaction credentials unavailable: TYPESAFE_API_KEY is not configured",
		);
	}
	try {
		const compaction = await generateJevCompaction({
		...
		const stats = compaction.details?.stats;
		if (stats) {
			onProgress?.(
				`jev: ${stats.kept} kept, ${stats.resultsDropped} results truncated, ${stats.callsDropped} calls dropped`,
			);
		}
		return compaction;
	} catch (error) {
		if (signal?.aborted) return undefined;
		if (error instanceof JevCompactionError) {
			if (error.kind === "aborted") return undefined;
			if (error.kind === "insufficient-reduction" || error.kind === "unfittable") {
				throw new SummaryGenerationError(
					"empty-summary",
					`Jev compaction produced no usable summary: ${error.message}`,
				);
			}
			throw new SummaryRequestError(`Jev compaction request failed: ${error.message}`, true);
		}
		throw error;
	}
}
